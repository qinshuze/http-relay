...
httpServer.get("/file/download", (req, res) => {
    ...
    const answerCallback = (event) => {
        ...
        fileInfo.writeSize = fileInfo.writeSize || 0
        fileInfo.writeSize = fileInfo.writeSize || 0

        fileList.set(localFilePath, fileInfo)

        const ETag = "W/\"" + (fileInfo.size + "").toString(16) + "-" + (fileInfo.updateTime + "").toString(16) + "\""
        const lastModified = new Date(fileInfo.updateTime).toUTCString()

        // res.setHeader("Cache-Control", "max-age=5");
        res.setHeader("Accept-Ranges", "bytes");
        res.setHeader("Last-Modified", new Date(fileInfo.updateTime).toUTCString());
        ...
        const ranges = []
        rangesStr.replaceAll(/\s+/, "").split(",").map((item, index) => {
            const tmpArr = item.split("-")
            ranges.push({start: tmpArr.shift() || 0, end: tmpArr.shift() || fileInfo.size})
        })

        res.setHeader('Content-Length', fileInfo.size)
        res.setHeader("Content-Type", fileInfo.mimeType)
        res.setHeader("Content-Range", `bytes ${ranges[0].start}-${ranges[0].end - 1}/${fileInfo.size}`)
        ...
        if (fileInfo.writeState === "write") {
            ...
            stream.on("data", (data) => {
                if (fileInfo.writeSize >= ranges[0].end) {
            ...
            const requestData = {
                ...
                file: (name, filestream, info) => {
                    ...
                    filestream.on("data", (data) => {
                        fileInfo.writeSize+=data.length
                        if (fileInfo.writeSize >= range.start && fileInfo.writeSize <= range.end && !resEnd) {
                            resEnd = true
                            const rs = fs.createReadStream(localFilePath, {start: range.start, end: range.end})
                            let rsDisconnect = true
                            rs.on("data", rData => res.write(rData))
                            rs.on("end", () => rsDisconnect = false)
                            rs.on("close", () => {
                                if (rsDisconnect) {
                                    res.status(500)
                                    console.log("读取流意外关闭")
